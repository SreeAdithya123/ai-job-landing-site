     ...
     >
       ...
       <header className="p-6 text-white" style={{ backgroundColor: theme.dark }}>
         <div className="flex items-center gap-2 mb-3">
           ...
           <span className="text-xs opacity-50 font-mono">resume.tsx</span>
         </div>
         <div className="font-mono">
           <p className="text-xs opacity-50">// Software Engineer</p>
           <h1 className="text-2xl font-bold mt-1">
             <span style={{ color: theme.primary }}>const</span> developer = "
             {data.personalInfo.fullName || 'Your Name'}"
           </h1>
         </div>
         <div className="flex flex-wrap gap-4 mt-4 text-xs font-mono opacity-80">
       ...
       <main className="p-6">
         ...
         {data.careerSummary && (
           <section className="mb-6 p-4 rounded-lg font-mono text-sm" style={{ backgroundColor: theme.light }}>
             <p className="text-gray-500">/**</p>
             <p className="text-gray-700 pl-2"> * {data.careerSummary}</p>
             <p className="text-gray-500"> */</p>
           </section>
         )}
 
         {/* Tech Stack */}
         {(data.skills.technologies.length > 0 || data.skills.tools.length > 0) && (
           <section className="mb-6">
             <h2 className="flex items-center gap-2 text-sm font-bold mb-3" style={{ color: theme.dark }}>
             ...
             <div className="grid grid-cols-3 gap-3">
               {data.skills.technologies.length > 0 && (
                 <div className="p-3 rounded-lg border" style={{ borderColor: theme.primary }}>
                   <div className="flex items-center gap-1 text-xs font-semibold mb-2" style={{ color: theme.primary }}>
                     <Code2 className="h-3 w-3" /> Languages
                   </div>
                   <div className="flex flex-wrap gap-1">
                     {data.skills.technologies.map((skill, i) => (
                       <span key={i} className="text-xs px-2 py-0.5 rounded bg-gray-100">{skill}</span>
                     ))}
                   </div>
                 </div>
               )}
         ...
         {/* Experience as Git Commits */}
         {data.experience.length > 0 && (
           <section className="mb-6">
             <h2 className="text-sm font-bold mb-3" style={{ color: theme.dark }}>
               Experience Log
             </h2>
               ...
               {data.experience.map((exp, idx) => (
                 <div key={exp.id} className="relative pl-6 pb-3 border-l-2" style={{ borderColor: theme.primary }}>
                   ...
                   <div className="flex justify-between items-start">
                     <div>
                       <span className="font-mono text-xs px-2 py-0.5 rounded" style={{ backgroundColor: theme.light, color: theme.dark }}>
                         v{data.experience.length - idx}.0
                       </span>
                       <h3 className="font-semibold mt-1">{exp.role}</h3>
                       <p className="text-sm" style={{ color: theme.primary }}>{exp.company}</p>
                     </div>
                     <span className="text-xs text-gray-400 font-mono">{exp.duration}</span>
                   </div>
             ...
             <div className="grid grid-cols-2 gap-3">
               {data.projects.map((project) => (
                 <div key={project.id} className="p-3 rounded-lg" style={{ backgroundColor: theme.light }}>
                   <h3 className="font-mono font-semibold text-sm" style={{ color: theme.dark }}>
                     📁 {project.title}
                   </h3>
                   ...
                   {project.technologies.length > 0 && (
                     <div className="flex flex-wrap gap-1 mt-2">
                       {project.technologies.map((tech, i) => (
                         <span key={i} className="text-xs px-1.5 py-0.5 rounded bg-white font-mono">
                           {tech}
                         </span>
                       ))}
                     </div>
         ...
         <div className="grid grid-cols-2 gap-6">
           {data.education.length > 0 && (
             <section>
               <h2 className="text-sm font-bold mb-3" style={{ color: theme.dark }}>Education</h2>
           ...
           {data.certifications.length > 0 && (
             <section>
               <h2 className="text-sm font-bold mb-3" style={{ color: theme.dark }}>Certifications</h2>
