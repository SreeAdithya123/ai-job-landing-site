    ...
    >
      ...
      <main className="p-6">
            ...
            <div className="grid grid-cols-3 gap-3">
              {data.skills.technologies.length > 0 && (
                <div className="p-3 rounded-lg border" style={{ borderColor: theme.primary }}>
                  <div className="flex items-center gap-1 text-xs font-semibold mb-2" style={{ color: theme.primary, fontFamily: headingFont }}>
        ...
        {data.experience.length > 0 && (
          <section className="mb-6">
            <h2 className="text-sm font-bold mb-3" style={{ color: theme.dark, fontFamily: headingFont }}>
              Experience Log
            </h2>
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      <span 
                        ...
                        style={{ backgroundColor: theme.light, color: theme.dark, fontFamily: monoFont }}
                      >
                  ...
                  {exp.responsibilities.length > 0 && (
                    <ul className="mt-2 space-y-1">
                      {exp.responsibilities.map((resp, i) => (
                        <li key={i} className="text-sm text-gray-600" style={{ fontFamily: monoFont }}>
            ...
            <div className="grid grid-cols-2 gap-3">
              {data.projects.map((project) => (
                <div key={project.id} className="p-3 rounded-lg" style={{ backgroundColor: theme.light }}>
                  <h3 className="font-semibold text-sm" style={{ color: theme.dark, fontFamily: monoFont }}>
                    📁 {project.title}
                  </h3>
                  ...
                  {project.technologies.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-2">
                      {project.technologies.map((tech, i) => (
                        <span key={i} className="text-xs px-1.5 py-0.5 rounded bg-white" style={{ fontFamily: monoFont }}>
                          {tech}
                        </span>
                      ))}
                    </div>
